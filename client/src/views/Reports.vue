...
export default {
  ...
  setup() {
    const { t } = useI18n()
    const { getCurrentFilters, selectedLocation, selectedPeriod } = useFilters()
    ...
    const calculateSummaryStats = () => {
      ...
      avgMonthlyRevenue.value = monthlyData.value.length > 0
      ...
      const best = quarterlyData.value.reduce(
        (b, q) => q.total_revenue > b.total_revenue ? q : b,
    ...
    onMounted(loadData)
    watch([selectedLocation, selectedPeriod], loadData)
